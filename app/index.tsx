...
type StoredState = { index?: number; history?: HistoryEntry[] };
...
function AnimatedWave({ wave, onDone }: { wave: Wave; onDone: (id: number) => void }) {
  ...
  useEffect(() => {
    Animated.timing(progress, { toValue: 1, duration: 1100, useNativeDriver: true }).start(({ finished }) => { if (finished) onDone(wave.id); });
...
function RippleTap() {
  ...
  const addWave = (x: number, y: number) => { const wave = { id: ++nextId.current, x, y }; setWaves((current) => [...current, wave]); softTick(); };
...
function ElasticScrub() {
  ...
  const pan = useMemo(() => PanResponder.create({
    ...
    onPanResponderMove: (_, g) => {
      const x = elastic(start.current.x + g.dx); const y = elastic(start.current.y + g.dy); position.setValue({ x, y });
      const band = Math.floor(Math.hypot(x, y) / 45); if (band !== lastBand.current) { lastBand.current = band; softTick(); } resetLater();
    },
...
function MockAd() {
  return <View style={[styles.experience, styles.ad]}><Text selectable={false} style={styles.adEyebrow}>ADVERTISEMENT SLOT</Text><Text selectable={false} style={styles.adTitle}>First ad only after 7 games.</Text><Text selectable={false} style={styles.adCopy}>POC placeholder — real ad SDK comes later.</Text></View>;
}
...
export default function Home() {
  ...
  useEffect(() => {
    AsyncStorage.getItem(STORAGE_KEY).then((raw) => {
      ...
      if (typeof saved.index === 'number' && saved.index > 0) { setItems(buildItems(0, Math.max(32, saved.index + 16))); setInitialIndex(saved.index); setCurrentIndex(saved.index); }
    }).finally(() => setReady(true));
  }, []);
  useEffect(() => { if (!ready || initialIndex === 0) return; requestAnimationFrame(() => listRef.current?.scrollToIndex({ index: initialIndex, animated: false })); }, [ready, initialIndex]);

  const persist = (index: number, nextHistory: HistoryEntry[]) => AsyncStorage.setItem(STORAGE_KEY, JSON.stringify({ index, history: nextHistory.slice(-40) })).catch(() => undefined);
  const goNext = () => {
    const now = Date.now();
    const current = items[currentIndex];
    const nextHistory = current?.kind === 'game' ? [...history, { id: current.id, label: GAME_LABELS[current.game], seconds: Math.max(0.1, (now - enteredAt.current) / 1000), at: now }].slice(-40) : history;
    const next = currentIndex + 1;
    if (next >= items.length - 6) setItems((old) => [...old, ...buildItems(old.length)]);
    setHistory(nextHistory); setCurrentIndex(next); enteredAt.current = now;
    listRef.current?.scrollToIndex({ index: next, animated: true }); impact(Haptics.ImpactFeedbackStyle.Medium); persist(next, nextHistory);
  };
...
const styles = StyleSheet.create({
  ...
  navSeparator: { position: 'absolute', top: 0, left: 0, right: 0, height: 2, backgroundColor: '#343840' }, navPill: { width: 42, height: 4, borderRadius: 2, backgroundColor: '#555b66', marginBottom: 10 }, navHint: { color: '#868c98', fontSize: 10, fontWeight: '800', letterSpacing: 1.4 },
  statsButton: { position: 'absolute', top: Platform.OS === 'web' ? 8 : 42, left: 10, zIndex: 30, paddingHorizontal: 9, paddingVertical: 5, borderRadius: 8, backgroundColor: 'rgba(255,255,255,0.06)' }, statsButtonText: { color: '#8b8e99', fontSize: 9, fontWeight: '800', letterSpacing: 0.7 },
  commitBadge: { position: 'absolute', top: Platform.OS === 'web' ? 8 : 42, right: 10, paddingHorizontal: 8, paddingVertical: 4, borderRadius: 8, backgroundColor: 'rgba(255,255,255,0.08)', zIndex: 30 }, commitText: { color: '#8b8e99', fontSize: 10, fontWeight: '700', letterSpacing: 0.5 },
  experience: { height: SCREEN_HEIGHT, backgroundColor: '#08090c', alignItems: 'center', paddingHorizontal: 24, paddingTop: Platform.OS === 'web' ? 40 : 72, paddingBottom: NAV_ZONE_HEIGHT + 12 }, copy: { alignItems: 'center', gap: 6, zIndex: 2 }, title: { color: '#f7f7f7', fontSize: 34, fontWeight: '900', letterSpacing: 1.5 }, subtitle: { color: '#8b8e99', fontSize: 14, letterSpacing: 0.5 }, gameArea: { flex: 1, width: '100%', alignItems: 'stretch', justifyContent: 'stretch', overflow: 'hidden' },
  ...
  centerPress: { width: '100%', height: '100%', alignItems: 'center', justifyContent: 'center' }, pressOrb: { width: 108, height: 108, borderRadius: 54, backgroundColor: '#f06cff', shadowColor: '#f06cff', shadowOpacity: 0.7, shadowRadius: 26 }, squishSurface: { backgroundColor: '#101719' }, squishBlob: { width: 150, height: 150, borderRadius: 75, backgroundColor: '#79f0c1', borderWidth: 7, borderColor: '#bcffe8', shadowColor: '#79f0c1', shadowOpacity: 0.65, shadowRadius: 24 },
  ...
  overlay: { ...StyleSheet.absoluteFillObject, zIndex: 100, backgroundColor: 'rgba(0,0,0,0.72)', alignItems: 'center', justifyContent: 'center', padding: 22 }, statsCard: { width: '100%', maxWidth: 420, backgroundColor: '#151820', borderRadius: 22, borderWidth: 1, borderColor: '#343947', padding: 20 }, statsHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }, statsTitle: { color: '#f4f4f5', fontSize: 18, fontWeight: '900', letterSpacing: 1 }, close: { color: '#a6abb6', fontSize: 22, padding: 4 }, statsSubtitle: { color: '#707582', fontSize: 11, marginBottom: 18 }, statRow: { flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 7, borderBottomWidth: 1, borderBottomColor: '#252a34' }, statName: { color: '#d6d8dd', fontWeight: '700' }, statValue: { color: '#9298a5' }, historyTitle: { color: '#777d89', fontSize: 10, fontWeight: '900', letterSpacing: 1.2, marginTop: 18, marginBottom: 7 }, historyLine: { color: '#aeb3bd', fontSize: 12, paddingVertical: 3 },
});
