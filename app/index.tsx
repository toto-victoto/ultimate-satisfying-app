...
import { Animated, Dimensions, FlatList, PanResponder, Platform, Pressable, StyleSheet, Text, View } from 'react-native';
...
const TAB_BAR_HEIGHT = Platform.OS === 'web' ? 64 : 76;
const FEED_HEIGHT = SCREEN_HEIGHT - TAB_BAR_HEIGHT;
const NAV_ZONE_HEIGHT = Math.max(86, Math.min(116, FEED_HEIGHT * 0.13));
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    AsyncStorage.getItem(STORAGE_KEY).then((raw) => {
      ...
      setMuted(savedMuted);
      setSynthMuted(savedMuted);
      const first = initialBatch[0];
      const now = Date.now();
      const sessionStart: HistoryEntry = first.kind === 'game'
        ? { id: `session-${now}`, game: first.game, label: GAME_LABELS[first.game], seconds: 0, at: now }
        : { id: `session-${now}`, game: 0, label: GAME_LABELS[0], seconds: 0, at: now };
      const nextHistory = [...(saved.history ?? []), sessionStart].slice(-100);
      setHistory(nextHistory);
      enteredAt.current = now;
      return AsyncStorage.setItem(STORAGE_KEY, JSON.stringify({ history: nextHistory, muted: savedMuted }));
  ...
  const goNext = () => {
    ...
    enteredAt.current = now;
    listRef.current?.scrollToOffset({ offset: next * FEED_HEIGHT, animated: true });
  ...
  return <View style={styles.root}>
    <FlatList
      ...
      renderItem={({ item }) => item.kind === 'ad' ? <MockAd /> : <Game game={item.game} />}
      scrollEnabled={false}
    ...
    <View style={[styles.bottomNavZone, { height: NAV_ZONE_HEIGHT }]} {...bottomNav.panHandlers}>
      ...
      <Text style={styles.navHint}>SWIPE UP · NEXT</Text>
    </View>
    <View pointerEvents="none" style={styles.commitBadge}><Text style={styles.commitText}>POC {COMMIT_SHA}</Text></View>
  </View>;
}
...
const styles = StyleSheet.create({
  ...
  experience: { height: FEED_HEIGHT, backgroundColor: '#08090c', alignItems: 'center', paddingHorizontal: 24, paddingTop: Platform.OS === 'web' ? 40 : 72, paddingBottom: NAV_ZONE_HEIGHT + 12 },
  copy: { alignItems: 'center', gap: 6, zIndex: 2 },
  title: { color: '#f7f7f7', fontSize: 34, fontWeight: '900', letterSpacing: 1.5 },
  subtitle: { color: '#8b8e99', fontSize: 14 },
  gameArea: { flex: 1, width: '100%', alignItems: 'stretch', justifyContent: 'stretch', overflow: 'hidden' },
  bottomNavZone: { position: 'absolute', left: 0, right: 0, bottom: 0, zIndex: 20, alignItems: 'center', justifyContent: 'center', paddingTop: 12 },
  ...
  commitText: { color: '#8b8e99', fontSize: 10, fontWeight: '700' },
