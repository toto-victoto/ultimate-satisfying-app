...
import { Dimensions, FlatList, PanResponder, Platform, StyleSheet, Text, View } from 'react-native';
...
export default function FeedPage() {
  ...
  const persistDeparture = async (current: FeedItem) => {
    ...
    } else state.stats.adsReached += 1;
    await writeState(state);
  };

  const goNext = () => {
    const current = items[currentIndex]; const next = currentIndex + 1;
    appendIfNeeded(next); setCurrentIndex(next); enteredAt.current = Date.now();
  ...
  return <View style={styles.root}><FlatList ref={listRef} data={items} keyExtractor={(item) => item.id} renderItem={({ item }) => <View style={{ height: ITEM_HEIGHT }}>{item.kind === 'ad' ? <MockAd /> : <SatisfyingGame game={item.game} />}</View>} scrollEnabled={false} initialNumToRender={3} maxToRenderPerBatch={4} windowSize={5} getItemLayout={(_, index) => ({ length: ITEM_HEIGHT, offset: ITEM_HEIGHT * index, index })} /><View style={[styles.nextZone, { height: NAV_ZONE_HEIGHT }]} {...nextPan.panHandlers}><View style={styles.separator} /><View style={styles.pill} /><Text style={styles.nextText}>SWIPE UP · NEXT</Text></View><View pointerEvents="none" style={styles.commit}><Text style={styles.commitText}>POC {COMMIT_SHA}</Text></View></View>;
}

const styles = StyleSheet.create({ root: { flex: 1, backgroundColor: '#08090c', ...(Platform.OS === 'web' ? ({ userSelect: 'none', WebkitUserSelect: 'none' } as object) : {}) }, nextZone: { position: 'absolute', left: 0, right: 0, bottom: 0, zIndex: 20, alignItems: 'center', justifyContent: 'center', paddingTop: 12 }, separator: { position: 'absolute', top: 0, left: 0, right: 0, height: 2, backgroundColor: '#343840' }, pill: { width: 42, height: 4, borderRadius: 2, backgroundColor: '#666c78', marginBottom: 10 }, nextText: { color: '#a1a6b1', fontSize: 10, fontWeight: '800', letterSpacing: 1.4 }, commit: { position: 'absolute', top: Platform.OS === 'web' ? 8 : 12, right: 10, paddingHorizontal: 8, paddingVertical: 4, borderRadius: 8, backgroundColor: 'rgba(255,255,255,0.08)', zIndex: 30 }, commitText: { color: '#a8adb7', fontSize: 10, fontWeight: '700' }, ad: { flex: 1, alignItems: 'center', justifyContent: 'center', gap: 18, backgroundColor: '#11131a' }, adEyebrow: { color: '#74798b', fontSize: 11, letterSpacing: 2 }, adTitle: { color: '#f5f5f5', fontSize: 30, fontWeight: '800' }, adCopy: { color: '#8b8e99', fontSize: 14 } });
