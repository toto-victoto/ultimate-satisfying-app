...
export default function FeedPage() {
  ...
  const nextPan = useMemo(() => PanResponder.create({
    ...
    onPanResponderRelease: (_, gesture) => { if (gesture.dy < -42 && navStartIndex.current === currentIndex) goNext(); },
  }), [currentIndex, items]);
  ...
  return <View style={styles.root}>
    <FlatList
      ...
      renderItem={({ item }) => <View style={{ height: ITEM_HEIGHT }}>{item.kind === 'ad' ? <MockAd /> : <SatisfyingGame game={item.game} />}</View>}
      scrollEnabled={false}
    ...
    <View style={[styles.nextZone, { height: NAV_ZONE_HEIGHT }]} {...nextPan.panHandlers}>
      <View style={styles.separator} /><View style={styles.pill} /><Text style={styles.nextText}>SWIPE UP · NEXT</Text>
    </View>
...
const styles = StyleSheet.create({
  ...
  nextZone: { position: 'absolute', left: 0, right: 0, bottom: 0, zIndex: 20, alignItems: 'center', justifyContent: 'center', paddingTop: 12 },
  ...
  adEyebrow: { color: '#74798b', fontSize: 11, letterSpacing: 2 }, adTitle: { color: '#f5f5f5', fontSize: 30, fontWeight: '800' }, adCopy: { color: '#8b8e99', fontSize: 14 },
});
